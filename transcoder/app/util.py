...
def transcode_to_mpeg_dash(s3_url, file, file_id, db):
    ...
    try:
        ...
        db.videos.update_one({"video_id": file_id}, {"$set": {"status": PROCESSING}})
        ...
        ffmpeg_cmd.extend(
            [
                "-init_seg_name",
                f"init_{file_id}_$RepresentationID$.mp4",
                "-media_seg_name",
                f"chunk_{file_id}_$Bandwidth$_$Number$.m4s",
            ]
        )
        ...
        db.videos.update_one({"video_id": file_id}, {"$set": {"status": PROCESSED}})
